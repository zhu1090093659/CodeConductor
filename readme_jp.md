...
- 会話ページの操作性を高める ActionToolbar を追加しました。
- CLI モデル一覧フックを追加し、手動選択の設定処理に対応しました。
- メッセージ処理とログ出力を改善し、デバッグしやすくしました。

### 1.7.8

- チャット UI コンポーネントを強化し、ビジュアルデザインとメッセージスタイルを改善しました。
- Superpowers ワークフローサービスを導入し、ワークフロー管理のためのガイド付きエージェント設定ページを追加しました。
- プルリクエストの Jira 説明を自動同期する GitHub Actions ワークフローを追加しました。
- コラボレーション UI を改善し、メッセージ表示とインタラクションパターンを洗練させました。

### 1.7.7

- Superpowers 統合を実装し、包括的な設定とコマンド管理機能を追加しました。
- Superpowers モードとワークフローを設定するための専用設定ページを追加しました。
- Superpowers フレームワーク統合によりプロジェクトワークフロー機能を強化しました。

### 1.7.6

- latest.yml メタデータファイルの適切な生成とアップロードにより、自動更新機能の 404 エラーを修正しました。
- 自動アプリケーション更新の信頼性を向上させました。

### 1.7.5

- 対話型プロンプトと確認のための組み込みツールとして popup-mcp を統合しました。
- 複数の言語ファイルによる国際化サポートを追加しました。
- Claude および Codex 用の CLI プロバイダー設定 UI とバックエンドサービスを追加しました。
- ストレージユーティリティと思考プロセス表示コンポーネントを追加しました。
- CLI プロバイダー設定での ANTHROPIC_MODEL のデフォルト値を修正しました。
- Guid コンポーネントでの重複するモデル設定呼び出しを修正しました。

### 1.7.3

- ACPアダプターを強化し、エージェントの思考をより良く表示可能なチャットメッセージに変換。
- CLIプロバイダー設定を実装し、CodexとClaudeのプリセットとストレージをサポート。
- 協業機能を改善し、マルチロールコミュニケーションと通知メカニズムをサポート。
- プロセス管理と孤児プロセスクリーンアップを最適化し、リソース処理能力を向上。
- IPC通信メカニズムを強化し、安定性を向上。
- electron-updater統合により自動増分更新を追加。
- GitHub Actions自動化ビルドとリリースパイプラインを改善。
