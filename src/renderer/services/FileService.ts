...
// 清理CodeConductor时间戳后缀，返回原始文件名
export function cleanCodeConductorTimestamp(fileName: string): string {
  return fileName.replace(CodeConductor_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(filePath: string): string {
  const fileName = filePath.split(/[\\/]/).pop() || '';
  return cleanCodeConductorTimestamp(fileName);
}
