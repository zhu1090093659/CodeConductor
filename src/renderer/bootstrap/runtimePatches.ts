...
const shouldSilence = (message?: string) => {
  ...
  return RESIZE_OBSERVER_PATTERNS.some((pattern) => normalized.includes(pattern)) || ARCO_POPUP_POSITION_PATTERNS.some((pattern) => normalized.includes(pattern)) || ARCO_MESSAGE_KEY_PATTERNS.some((pattern) => normalized.includes(pattern)) || REACT_19_REF_PATTERNS.some((pattern) => normalized.includes(pattern));
};
